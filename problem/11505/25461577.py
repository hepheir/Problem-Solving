...
def make(start, end, node):
    if start == end:
        ...
        mid = (start+end)//2
        tree[node] = ((make(start, mid, node*2) % MOD) * (make(mid+1, end, node*2+1)) % MOD) % MOD
    return tree[node]
...
def update(start, end, node, index, value):
    if index == start == end:
        ...
        mid = (start+end)//2
        tree[node] = ((update(start, mid, node*2, index, value) % MOD) * (update(mid+1, end, node*2+1, index, value) % MOD)) % MOD
    return tree[node]
